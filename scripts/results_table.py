...
six numbers rather than the ones that favour a hypothesis, and a filter flag
...
def format_table(summaries, columns) -> str:
    ...
    for tag, summary in summaries.items():
        ...
        for benchmark, metric in columns:
            ...
            if best.get((benchmark, metric)) == value and len(summaries) > 1:
                text = f"**{text}**"
            row.append(text)
...
def main():
    ...
    document = "\n".join([
        ...
        "",
        format_table(summaries, columns),
